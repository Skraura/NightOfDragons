...
const NAV_ADMIN = [
  { id: 'clan-canvas', label: 'Clan Canvas', Icon: ClanCanvasIcon },
    ...
    <aside className={styles.sidebar}>
      <div className={styles.user}>
        ...
        <div className={styles.userInfo}>
          <span className={styles.username}>
            {user.username}
            {user.isAdmin && <span className={styles.adminBadge} title="Admin">★</span>}
          </span>
          <span className={styles.userSub}>
            {user.isAdmin ? 'Administrator' : 'Dragon Keeper'}
      ...
      <nav className={styles.nav}>
        {NAV.map(({ id, label, Icon }) => (
          <button
            key={id}
            className={`${styles.navBtn} ${view === id ? styles.navActive : ''}`}
            onClick={() => onView(id)}
          >
            <Icon />
            <span>{label}</span>
          ...
          <>
            ...
            {NAV_ADMIN.map(({ id, label, Icon }) => (
              <button
                ...
                onClick={() => onView(id)}
                title="Admin-only: shared clan lineage canvas"
              >
                <Icon />
                <span>{label}</span>
              </button>
...
function TrainingIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>
}
